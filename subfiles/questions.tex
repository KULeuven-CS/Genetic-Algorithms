...
\begin{enumerate}
    ...
    It is important that the fitness function (1) rewards the right kind of individuals and (2) can be calculated quickly.  The fitness can be approximated when the computation time is extremely high or the objective function is noisy or uncertain. Another commonly used approach is the introduction of a performance function $u(x) = f(x) - f_\text{min}$ that guarantees the performance to be positive. Because this makes values hard to distinguish (leading to reduced selection pressure and stagnation). A solution is to use $f'_\text{min}$ which is the minimal of the last generation. This approach is called scaling and is guided by a parameter called the scaling window.}
